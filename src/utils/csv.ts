import type { Holding, Trade } from '../state/types';
...
const stripBom = (value: string): string => value.replace(/^\ufeff+/g, '');
...
const normaliseBoolean = (raw: string | undefined): boolean => {
...
const normaliseAssetType = (raw: string | undefined): HoldingCsvRow['assetType'] => {
  const allowed: HoldingCsvRow['assetType'][] = ['ETF', 'Stock', 'Crypto', 'Cash', 'Bond', 'Fund', 'Other'];
  if (!raw) {
    return 'Other';
  }
...
const parseSpecCsv = (csv: string): HoldingCsvRow[] | undefined => {
  const lines = csv
    .split(/\r?\n/)
...
const parseInteractiveInvestorCsv = (csv: string): HoldingCsvRow[] | undefined => {
  const lines = csv.split(/\r?\n/).map((line) => stripBom(line.trim())).filter((line) => line.length > 0);
  if (lines.length === 0) {
    return [];
  }
...
const parseHlCsv = (csv: string): HoldingCsvRow[] | undefined => {
